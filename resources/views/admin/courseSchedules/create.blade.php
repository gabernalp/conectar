...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.courseSchedule.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.course-schedules.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tutors') ? 'is-invalid' : '' }}" name="tutors[]" id="tutors" multiple required>
                    @foreach($tutors as $id => $tutors)
                        <option value="{{ $id }}" {{ in_array($id, old('tutors', [])) ? 'selected' : '' }}>{{ $tutors }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('tutor_capacity') ? 'is-invalid' : '' }}" type="number" name="tutor_capacity" id="tutor_capacity" value="{{ old('tutor_capacity', '10') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('iterations_number') ? 'is-invalid' : '' }}" type="text" name="iterations_number" id="iterations_number" value="{{ old('iterations_number', '') }}">
