    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.courses-hooks.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('requirements') ? 'is-invalid' : '' }}" name="requirements" id="requirements">{{ old('requirements') }}</textarea>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('priorized') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="priorized" id="priorized" value="1" {{ old('priorized', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('rols') ? 'is-invalid' : '' }}" name="rols[]" id="rols" multiple>
                    @foreach($rols as $id => $rol)
                        <option value="{{ $id }}" {{ in_array($id, old('rols', [])) ? 'selected' : '' }}>{{ $rol }}</option>
                    @endforeach
                </select>
                @if($errors->has('rols'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('growinghook') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="growinghook" id="growinghook" value="1" {{ old('growinghook', 0) == 1 ? 'checked' : '' }}>
