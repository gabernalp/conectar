        ...
        <div class="col-md-12">
            @can('user_chain_block_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.user-chain-blocks.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-UserChainBlock">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $userChainBlock->id }}">
                                        ...
                                        <td>
                                            @can('user_chain_block_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.user-chain-blocks.show', $userChainBlock->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('user_chain_block_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.user-chain-blocks.edit', $userChainBlock->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('user_chain_block_delete')
                                                <form action="{{ route('frontend.user-chain-blocks.destroy', $userChainBlock->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('user_chain_block_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.user-chain-blocks.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-UserChainBlock:not(.ajaxTable)').DataTable({ buttons: dtButtons })
