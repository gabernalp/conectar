    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.challenges.update", [$challenge->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="courses[]" id="courses" multiple>
                                @foreach($courses as $id => $courses)
                                    <option value="{{ $id }}" {{ (in_array($id, old('courses', [])) || $challenge->courses->contains($id)) ? 'selected' : '' }}>{{ $courses }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $challenge->name) }}" required>
                            @if($errors->has('name'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('name') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="description" id="description" value="{{ old('description', $challenge->description) }}">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="goal" id="goal">{{ old('goal', $challenge->goal) }}</textarea>
                            @if($errors->has('goal'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('goal') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="capsule" id="capsule" value="{{ old('capsule', $challenge->capsule) }}">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="capsule_content" id="capsule_content">{{ old('capsule_content', $challenge->capsule_content) }}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <div class="needsclick dropzone" id="capsule_file-dropzone">
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="challenge_action" id="challenge_action">
                                <option value disabled {{ old('challenge_action', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                                @foreach(App\Challenge::CHALLENGE_ACTION_SELECT as $key => $label)
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="action_detail" id="action_detail">{{ old('action_detail', $challenge->action_detail) }}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="limit_time" id="limit_time">
                                <option value disabled {{ old('limit_time', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                                @foreach(App\Challenge::LIMIT_TIME_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('limit_time', $challenge->limit_time) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="referencetype_id" id="referencetype_id" required>
                                @foreach($referencetypes as $id => $referencetype)
                                    <option value="{{ $id }}" {{ (old('referencetype_id') ? old('referencetype_id') : $challenge->referencetype->id ?? '') == $id ? 'selected' : '' }}>{{ $referencetype }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="hours_adding" id="hours_adding" value="{{ old('hours_adding', $challenge->hours_adding) }}" step="1" required>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="points[]" id="points" multiple>
                                @foreach($points as $id => $points)
                                    <option value="{{ $id }}" {{ (in_array($id, old('points', [])) || $challenge->points->contains($id)) ? 'selected' : '' }}>{{ $points }}</option>
                                @endforeach
                            </select>
...
<script>
    ...
    url: '{{ route('frontend.challenges.storeMedia') }}',
    maxFilesize: 10, // MB
    maxFiles: 1,
    addRemoveLinks: true,
    headers: {
