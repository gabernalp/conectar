        ...
        <div class="col-md-12">
            @can('entity_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.entities.create') }}">
                            ...
                            <tbody>
                                @foreach($entities as $key => $entity)
                                    <tr data-entry-id="{{ $entity->id }}">
                                        ...
                                        <td>
                                            @can('entity_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.entities.show', $entity->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('entity_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.entities.edit', $entity->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('entity_delete')
                                                <form action="{{ route('frontend.entities.destroy', $entity->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('entity_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.entities.massDestroy') }}",
