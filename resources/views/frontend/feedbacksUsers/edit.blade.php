        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.edit') }} {{ trans('cruds.feedbacksUser.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.feedbacks-users.update", [$feedbacksUser->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="user_id" id="user_id" required>
                                @foreach($users as $id => $user)
                                    <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $feedbacksUser->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('user'))
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="feedbacktype_id" id="feedbacktype_id" required>
                                @foreach($feedbacktypes as $id => $feedbacktype)
                                    <option value="{{ $id }}" {{ (old('feedbacktype_id') ? old('feedbacktype_id') : $feedbacksUser->feedbacktype->id ?? '') == $id ? 'selected' : '' }}>{{ $feedbacktype }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="referencetype_id" id="referencetype_id" required>
                                @foreach($referencetypes as $id => $referencetype)
                                    <option value="{{ $id }}" {{ (old('referencetype_id') ? old('referencetype_id') : $feedbacksUser->referencetype->id ?? '') == $id ? 'selected' : '' }}>{{ $referencetype }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="description" id="description">{{ old('description', $feedbacksUser->description) }}</textarea>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="link" id="link" value="{{ old('link', $feedbacksUser->link) }}">
                            @if($errors->has('link'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('link') }}
                                </div>
