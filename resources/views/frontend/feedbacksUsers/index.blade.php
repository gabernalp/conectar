        ...
        <div class="col-md-12">
            @can('feedbacks_user_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.feedbacks-users.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-FeedbacksUser">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $feedbacksUser->id }}">
                                        ...
                                        <td>
                                            @can('feedbacks_user_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.feedbacks-users.show', $feedbacksUser->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('feedbacks_user_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.feedbacks-users.edit', $feedbacksUser->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('feedbacks_user_delete')
                                                <form action="{{ route('frontend.feedbacks-users.destroy', $feedbacksUser->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('feedbacks_user_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.feedbacks-users.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-FeedbacksUser:not(.ajaxTable)').DataTable({ buttons: dtButtons })
