    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.background-processes.update", [$backgroundProcess->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="name">{{ trans('cruds.backgroundProcess.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $backgroundProcess->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="description" id="description" required>{{ old('description', $backgroundProcess->description) }}</textarea>
                        ...
                        <div class="form-group">
                            <label for="tags">{{ trans('cruds.backgroundProcess.fields.tags') }}</label>
                            ...
                            <select class="form-control select2" name="tags[]" id="tags" multiple>
                                @foreach($tags as $id => $tags)
                                    <option value="{{ $id }}" {{ (in_array($id, old('tags', [])) || $backgroundProcess->tags->contains($id)) ? 'selected' : '' }}>{{ $tags }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('tags'))
                                <div class="invalid-feedback">
                            ...
                            @endif
                            <span class="help-block">{{ trans('cruds.backgroundProcess.fields.tags_helper') }}</span>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="comments" id="comments">{{ old('comments', $backgroundProcess->comments) }}</textarea>
...
Dropzone.options.fileDropzone = {
    url: '{{ route('frontend.background-processes.storeMedia') }}',
    maxFilesize: 5, // MB
    addRemoveLinks: true,
    headers: {
