        ...
        <div class="col-md-12">
            @can('course_schedule_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.course-schedules.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-CourseSchedule">
                            <thead>
                                <tr>
                                    <th>
                            ...
                            <tbody>
                                @foreach($courseSchedules as $key => $courseSchedule)
                                    <tr data-entry-id="{{ $courseSchedule->id }}">
                                        ...
                                        <td>
                                            @foreach($courseSchedule->tutors as $key => $item)
                                                <span>{{ $item->name }}</span>
                                            @endforeach
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            @can('course_schedule_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.course-schedules.show', $courseSchedule->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('course_schedule_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.course-schedules.edit', $courseSchedule->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('course_schedule_delete')
                                                <form action="{{ route('frontend.course-schedules.destroy', $courseSchedule->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('course_schedule_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.course-schedules.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-CourseSchedule:not(.ajaxTable)').DataTable({ buttons: dtButtons })
