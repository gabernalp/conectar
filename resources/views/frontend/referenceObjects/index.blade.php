        ...
        <div class="col-md-12">
            @can('reference_object_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.reference-objects.create') }}">
                            {{ trans('global.add') }} {{ trans('cruds.referenceObject.title_singular') }}
                        </a>
                        <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                            {{ trans('global.app_csvImport') }}
                        </button>
                        @include('csvImport.modal', ['model' => 'ReferenceObject', 'route' => 'admin.reference-objects.parseCsvImport'])
                    </div>
                </div>
            @endcan
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-ReferenceObject">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $referenceObject->id }}">
                                        ...
                                        <td>
                                            @can('reference_object_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.reference-objects.show', $referenceObject->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('reference_object_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.reference-objects.edit', $referenceObject->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('reference_object_delete')
                                                <form action="{{ route('frontend.reference-objects.destroy', $referenceObject->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('reference_object_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.reference-objects.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-ReferenceObject:not(.ajaxTable)').DataTable({ buttons: dtButtons })
