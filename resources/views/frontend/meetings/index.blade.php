        ...
        <div class="col-md-12">
            @can('meeting_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.meetings.create') }}">
                            ...
                            <tbody>
                                @foreach($meetings as $key => $meeting)
                                    <tr data-entry-id="{{ $meeting->id }}">
                                        ...
                                        <td>
                                            @can('meeting_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.meetings.show', $meeting->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('meeting_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.meetings.edit', $meeting->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('meeting_delete')
                                                <form action="{{ route('frontend.meetings.destroy', $meeting->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('meeting_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.meetings.massDestroy') }}",
