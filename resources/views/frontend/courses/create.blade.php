                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.courses.store") }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="associated_processes[]" id="associated_processes" multiple required>
                                @foreach($associated_processes as $id => $associated_processes)
                                    <option value="{{ $id }}" {{ in_array($id, old('associated_processes', [])) ? 'selected' : '' }}>{{ $associated_processes }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="goal" id="goal" required>{{ old('goal') }}</textarea>
                            @if($errors->has('goal'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('goal') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="focalizacion_territorials[]" id="focalizacion_territorials" multiple required>
                                @foreach($focalizacion_territorials as $id => $focalizacion_territorial)
                                    <option value="{{ $id }}" {{ in_array($id, old('focalizacion_territorials', [])) ? 'selected' : '' }}>{{ $focalizacion_territorial }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            <div>
                                ...
                                <input type="checkbox" name="support_required" id="support_required" value="1" {{ old('support_required', 0) == 1 || old('support_required') === null ? 'checked' : '' }}>
                                <label for="support_required">{{ trans('cruds.course.fields.support_required') }}</label>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="operators[]" id="operators" multiple>
                                @foreach($operators as $id => $operators)
                                    <option value="{{ $id }}" {{ in_array($id, old('operators', [])) ? 'selected' : '' }}>{{ $operators }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="references[]" id="references" multiple>
                                @foreach($references as $id => $references)
                                    <option value="{{ $id }}" {{ in_array($id, old('references', [])) ? 'selected' : '' }}>{{ $references }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="courseshooks[]" id="courseshooks" multiple>
